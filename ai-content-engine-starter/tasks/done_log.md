...
- V3-013 completed: added a deterministic contextual variant selector for existing A/B drafts that uses channel/topic/hour performance signals, returns a selected draft ID, and falls back to configured/default variant rules without experimentation infrastructure.
- V3-014 completed: added minimal channel_relationships storage (schema, domain type/repository, PostgreSQL create/list methods, and focused validation/migration tests) for static parent/sibling/promotion_target links without scheduling or planner integration.
- V3-015 completed: added a minimal network-aware scheduler retry policy with bounded per-tick attempts, backoff support, retryable error typing, and focused tests for transient versus permanent failures.
- V3-016 completed: added a standalone cross-channel topic balancing service that evaluates publish intents using normalized topic/cluster signals, simple per-window limits, channel priorities, cooldown delays, and focused allow/delay/deprioritize tests without analytics or personalization dependencies.
- V3-017 completed: added minimal sponsors storage (schema, domain type/repository, PostgreSQL create/get/list methods, and focused migration/repository validation tests) without campaigns, billing, or pipeline integration.
- V3-018 completed: added minimal ad_campaigns storage linked to sponsors (schema, domain type/repository, PostgreSQL create/get/list methods, and focused migration/repository validation tests) without delivery logic, targeting, or publishing integration.
- V3-019 completed: added explicit ad_slots scheduling storage (schema, domain type/repository, PostgreSQL create/list-by-channel methods, and focused migration/repository validation tests) without editorial coupling, auto-placement, or publishing integration.
